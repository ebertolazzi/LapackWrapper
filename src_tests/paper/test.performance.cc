...
static void out( unsigned const   N,
                 unsigned const   nnz,
                 Real     const & ta,
                 Real     const & tb,
                 Real     const & tc ) {
...
main() {
  ...
  for ( ; *p != NULL ; ++p ) {
    ...
    BLAS_duscr_insert_entries( mHandle,  ccoor . nnz(),
                               &ccoor . getA()(0),
                               (int*)&ccoor . getI()(0),
                               (int*)&ccoor . getJ()(0) );
    ...
    for ( unsigned i = 0 ; i < R . size() ; ++i ) ++R[i] ;
    for ( unsigned i = 0 ; i < J . size() ; ++i ) ++J[i] ;

    unsigned const n = ccoor . numRows() ;
    ...
    unsigned cicle_repeat = 1 + 1000000 / n ;
