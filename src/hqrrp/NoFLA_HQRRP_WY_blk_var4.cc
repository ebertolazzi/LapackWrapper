...
#include <stdio.h>
#include <math.h>
#include "NoFLA_HQRRP_WY_blk_var4.hh"
...
namespace lapack_wrapper {
  ...
  static
  int
  NoFLA_Normal_random_matrix( int m_A, int n_A, double * buff_A, int ldim_A );

  static
  double
  NoFLA_Normal_random_number( double mu, double sigma );
  ...
  static
  int
  NoFLA_QRPmod_WY_unb_var4(
    int pivoting, int num_stages,
  ...
  static
  int
  NoFLA_QRP_downdate_partial_norms(
    int m_A, int n_A,
  ...
  dgeqp4(
    ...
    info = 0;
    integer lquery = (lwork == -1);
    if      ( m_A    < 0          ) info = -1;
    else if ( n_A    < 0          ) info = -2;
    else if ( ldim_A < max(1,m_A) ) info = -4;
    ...
    if ( info == 0 ) {
      if( mn_A == 0 ) {
        ...
        iws    = 3 * n_A + 1;
        nb     = lapack_wrapper::ilaenv( INB, "DGEQRF", " ", m_A, n_A, 6, 1 );
        lwkopt = 2 * n_A + ( n_A + 1 ) * nb;
      }
      work[ 0 ] = ( double ) lwkopt;

      if ( (lwork < iws ) && ( !lquery ) ) info = -8;
    }
    ...
    //  geqpf( m_A, n_A, A, lda, jpvt, tau, work, info );
    //  return;
    //} else
    if( mn_A < THRESHOLD_FOR_DGEQP3 ) {
      //// printf( "Calling dgeqp3\n" );
      info = geqp3( m_A, n_A, A, ldim_A, jpvt, tau, work, lwork );
      return;
    }
    ...
    for ( j = 0; j < n_A; j++ ) {
      if ( jpvt[ j ] != 0 ) {
        if ( j != num_fixed_cols ) {
          ...
          swap( m_A, & A[ 0 + j              * ldim_A ], 1,
                     & A[ 0 + num_fixed_cols * ldim_A ], 1 );
    ...
    if ( num_factorized_fixed_cols > 0 ) {
      info = geqrf( m_A, num_factorized_fixed_cols, A, ldim_A, tau, work, lwork );
      if ( info != 0 )
        fmt::print( stderr, "ERROR in dgeqrf: Info: {}\n", info );
      ...
      if ( num_factorized_fixed_cols < n_A ) {
        ...
        if ( info != 0 )
          fmt::print( stderr, "ERROR in dormqr: Info: {}\n", info );

        iws = max( iws, static_cast<integer>(work[0]) );
      }
    }
    ...
    if ( num_factorized_fixed_cols > 0 ) {
      // Copy vector.
      std::copy_n( jpvt, n_A, previous_jpvt );
    }
  ...
  NoFLA_HQRRP_WY_blk_var4(
    ...
    // Check arguments.
    if ( m_A < 0 ) {
      cerr << "ERROR in NoFLA_HQRRP_WY_blk_var4: m_A is < 0.\n";
    } if( n_A < 0 ) {
      cerr << "ERROR in NoFLA_HQRRP_WY_blk_var4: n_A is < 0.\n";
    } if( ldim_A < max( 1, m_A ) ) {
      cerr << "ERROR in NoFLA_HQRRP_WY_blk_var4: ldim_A is < max( 1, m_A ).\n";
    }

    // Some initializations.
    integer   mn_A   = min( m_A, n_A );
    integer * buff_p = buff_jpvt;
    buff_s = buff_tau;
    ...
    Malloc<double> mem("NoFLA_HQRRP_WY_blk_var4");

    // Create auxiliary objects.
    integer m_Y = nb_alg + pp;
    integer n_Y = n_A;
    integer m_V = nb_alg + pp;
    integer n_V = n_A;
    integer m_W = nb_alg;
    integer n_W = n_A;
    integer m_G = nb_alg + pp;
    integer n_G = m_A;

    mem.allocate( m_Y * n_Y + m_V * n_V + m_W * n_W + m_G * n_G );

    double * buff_Y  = mem( m_Y * n_Y );
    integer  ldim_Y  = m_Y;

    double * buff_V  = mem( m_V * n_V );
    integer  ldim_V  = m_V;

    double * buff_W  = mem( m_W * n_W );
    integer  ldim_W  = m_W;

    double * buff_G  = mem( m_G * n_G );
    integer  ldim_G  = m_G;
    ...
    gemm(
      ...
      m_Y, n_Y, m_A,
      1.0, buff_G, ldim_G, buff_A, ldim_A,
     0.0, buff_Y, ldim_Y
    );
    ...
    for( j = 0; j < mn_A; j += nb_alg ) {
      ...
      if ( last_iter == 0 ) {
        ...
        gecopy( m_V, n_VR, buff_YR, ldim_Y, buff_VR, ldim_V );
        NoFLA_QRPmod_WY_unb_var4(
          1, b,
  ...
  NoFLA_Normal_random_matrix(
    ...
    //
    // Main loop.
    for ( integer j = 0; j < n_A; ++j ) {
      for ( integer i = 0; i < m_A; ++i ) {
  ...
  NoFLA_Downdate_Y(
    ...
    // B = G1.
    //// FLA_Copy( G1, B );
    gecopy( m_G1, n_G1, buff_G1, ldim_G1, buff_B, ldim_B );
    ...
    ////           FLA_ONE, U11, B );
    trmm(
      RIGHT, LOWER, NO_TRANSPOSE, UNIT,
      m_B, n_B, 1.0, buff_U11, ldim_U11, buff_B, ldim_B
    );
    ...
    gemm(
      NO_TRANSPOSE, NO_TRANSPOSE,
      m_B, n_B, m_U21,
      1.0, buff_G2, ldim_G2, buff_U21, ldim_U21,
      1.0, buff_B, ldim_B
    );
    ...
    trmm(
      ...
      m_B, n_B, 1.0, buff_T, ldim_T, buff_B, ldim_B
    );
    ...
    ////           FLA_MINUS_ONE, U11, B );
    trmm(
      RIGHT, LOWER, TRANSPOSE /*"Conj_tranpose"*/, UNIT,
      m_B, n_B,
      -1.0, buff_U11, ldim_U11, buff_B, ldim_B
    );

    // B = G1 + B.
    //// FLA_Axpy( FLA_ONE, G1, B );
    for ( integer j = 0; j < n_B; j++ ) {
      for ( integer i = 0; i < m_B; i++ ) {
    ...
    gemm(
      NO_TRANSPOSE, NO_TRANSPOSE,
      m_Y2, n_Y2, m_A12,
      -1.0, buff_B, ldim_B, buff_A12, ldim_A12,
       1.0, buff_Y2, ldim_Y2
    );
  ...
  NoFLA_Apply_Q_WY_rnfc_blk_var4(
    ...
    // Apply the block transformation.
    larfb(
      RIGHT, NO_TRANSPOSE, FORWARD, COLUMNWISE,
      m_B, n_B, n_U, buff_U, ldim_U, buff_T, ldim_T,
      buff_B, ldim_B, buff_W, ldim_W
    );
    return 0;
  ...
  NoFLA_QRPmod_WY_unb_var4(
    ...
    //
    int     m_a21, m_A22, n_A22, n_dB, idx_max_col,
    ...
    if ( num_stages < 0 ) num_stages = mn_A;

    // Create auxiliary vectors.
    Malloc<double> mem("NoFLA_QRPmod_WY_unb_var4");
    mem.allocate( 3 * n_A );
    double * buff_d         = mem( n_A );
    double * buff_e         = mem( n_A );
    double * buff_workspace = mem( n_A );
    ...
    // Main Loop.
    for( integer j = 0; j < num_stages; j++ ) {
      n_dB  = n_A - j;
      m_a21 = m_A - j - 1;
      m_A22 = m_A - j - 1;
      n_A22 = n_A - j - 1;
      ...
      if ( pivoting == 1 ) {
        ...
        idx_max_col = iamax( n_dB, buff_d+j, 1 );
      ...
      larfg(
        n_house_vector,
        buff_A[ j + j * ldim_A ],
        & buff_A[ min( m_A-1, j+1 ) + j * ldim_A ], 1,
        & buff_t[ j ]
      );
      ...
      m_rest = m_A22 + 1;
      larf(
        lapack_wrapper::LEFT, m_rest, n_A22,
        & buff_A[ j + j * ldim_A ], 1,
        & buff_t[ j ],
    ...
    if ( build_T )
      larft(
        FORWARD, COLUMNWISE, m_A, num_stages, buff_A, ldim_A,
  ...
  NoFLA_QRP_downdate_partial_norms(
    ...
    */
    ...
    // Some initializations.
    tol3z  = sqrt( lamch<double>( "Epsilon" ) );
    ptr_d  = buff_d;
    ptr_e  = buff_e;
    ptr_wt = buff_wt;
    ptr_A  = buff_A;
